...
export function usePaymentSelection() {
    const [selectedPayment, setSelectedPayment] = useState<SelectedPaymentInfo | null>(null);
...
export function usePaymentProcessing() {
     ...
     */
    const processCardPayment = async (stripePaymentMethodId: string, totalAmountRands: number): Promise<boolean> => {
        ...
        try {
            ...
            const result = await createPaymentIntent(amountInCents, stripePaymentMethodId, 'zar');
            ...
            if (result.requiresAction && result.clientSecret) {
                ...
                if (actionError) {
                    Alert.alert('Authentication Failed', actionError.message || 'Card authentication failed');
                    return false;
                }
