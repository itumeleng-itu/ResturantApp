                ...
                <ScrollView className="flex-1 p-6">
                    <View className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
                        <View className="flex-row items-center justify-between mb-4">
                            ...
                            <Text className="text-gray-500 text-sm">#{activeJob.id.slice(0, 8)}</Text>
                        </View>
                        ...
                        <View className="flex-row items-start mb-4">
                             ...
                             <Text className="text-gray-600 ml-2 flex-1 text-base">
                                {activeJob.delivery_street}, {activeJob.delivery_city}, {activeJob.delivery_postal_code}
                             </Text>
                        </View>

                        <View className="border-t border-gray-100 my-4 pt-4">
                            <Text className="font-semibold text-gray-700 mb-2">Order Items ({activeJob.num_items})</Text>
                            {/* Simple list of items if needed, or just summary */}
                            <Text className="text-gray-500">{activeJob.unique_items} unique items</Text>
                            <Text className="text-xl font-bold text-gray-800 mt-2">Total: R{activeJob.total.toFixed(2)}</Text>
                        </View>

                        <View className="bg-blue-50 p-4 rounded-xl mt-4">
                             <Text className="text-blue-800 font-medium mb-2">Customer Contact</Text>
                             <Text className="text-blue-600 text-lg font-bold">{activeJob.user_contact}</Text>
                        </View>
                    </View>

                    <View className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                        <Text className="text-lg font-bold text-gray-800 mb-4">Complete Delivery</Text>
                        <Text className="text-gray-500 mb-4">Enter the PIN provided by the customer to complete this order.</Text>
    ...
    return (
        <View className="flex-1 bg-gray-50" style={{ paddingTop: insets.top }}>
            <View className="px-6 py-4 bg-white shadow-sm flex-row justify-between items-center z-10">
                <View>
                    <Text className="text-2xl font-bold text-gray-800">Available Jobs</Text>
                    <Text className="text-gray-500 text-sm">{availableJobs.length} orders nearby</Text>
                </View>
            ...
            <ScrollView 
                ...
                refreshControl={<RefreshControl refreshing={jobsLoading} onRefresh={refreshJobs} />}
            >
                {availableJobs.length === 0 ? (
                    <View className="items-center justify-center py-20">
                        <MaterialIcons name="hourglass-empty" size={64} color="#d1d5db" />
                        <Text className="text-gray-400 mt-4 text-center">No jobs available right now.</Text>
                        ...
                        <View key={job.id} className="bg-white rounded-2xl p-5 mb-4 shadow-sm border border-gray-100">
                             <View className="flex-row justify-between items-start mb-3">
                                <View className="flex-1">
                                    ...
                                    <Text className="text-gray-500 text-sm">{job.delivery_city}</Text>
                                </View>
                                <View className="bg-orange-100 px-3 py-1 rounded-full">
                                    <Text className="text-orange-700 font-bold">R{job.delivery_fee.toFixed(2)} Fee</Text>
                                </View>
                             </View>
                             ...
                             <View className="flex-row items-center mb-4">
                                ...
                                <Text className="text-gray-500 ml-1 text-xs">{job.num_items} items</Text>
                             </View>
                             ...
                             <View className="flex-row gap-3">
                                 ...
                                 >
                                     <MaterialIcons name="info-outline" size={24} color="#4b5563" />
                                 </TouchableOpacity>
                             </View>
                        </View>
                    ))
